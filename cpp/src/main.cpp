...
#define SAVE_OUTPUT_VIDEO false
...
int main(int argc, char** argv)
{
	if (argc != 4)
	{
		printf("usage: DetectParking <Laplacian> <Video_Filename or Camera_Number> <ParkingData_Filename>\n\n");
                cout << "<Laplacian> a float value to detect occupied parking, default is 2.0" << endl;
    ...
    ConfigLoad::parse();
        ...
        const string park_laplacian = argv[1];
	const string videoFilename = argv[2];	
	vector<Parking>  parking_data = parse_parking_file(argv[3]);
	const int delay = 1;	
	...
	while (cap.isOpened())
	{
		...
		if (DETECT_PARKING)
		{
			for (Parking& park : parking_data)
			{
                ...
                park.setStatus( delta[0] < atof(park_laplacian.c_str()) );
